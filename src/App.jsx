...
const App = () => {
  ...
  useEffect(() => {
    const scroll = new LocomotiveScroll({
      el: document.querySelector("#root"), // Or your scroll container
      smooth: true,
    });
  ...
  return (
    <>
      ...
      <span
    ref={growingSpan}
        className="growing rounded-full text-white block fixed top-[-20px] left-[-20px] w-5 h-5"
        ></span>
      <div className="w-full relative text-white min-h-screen font-['Helvetica_Now_Display']">
        {showCanvas &&
          data[0].map((canvasdets, index) => <Canvas key={index} details={canvasdets} />)}
        <div className="w-full relative z-[1] h-screen ">
          <nav className="w-full p-8 flex justify-between z-50">
            <div className="brand text-2xl font-md">DreamWeaversStudios</div>
            <div className="text-white links flex gap-10">
              {[
                "What we do",
                "Who we are",
                "How we give back",
          ...
          <div className="textcontainer  w-full px-[20%]">
            <div className="text w-[50%]">
              <h3 className="text-4xl leading-[1.2]">
                At DreamWeaversStudios, we build immersive digital experiences for
              ...
              <p className="text-md mt-10">CLICK FOR A MAGIC TRICK !!</p>
            </div>
          </div>
